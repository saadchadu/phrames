...
const { signup, loginWithGoogle, loading, initAuth } = useAuth()
...
const handleSignup = async () => {
  ...
  if (form.password !== form.confirmPassword) {
    formErrors.value = ["Passwords don't match"]
    return
  }
...
const handleGoogleSignup = async () => {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : 'Failed to create account with Google. Please try again.'
    formErrors.value = [message]
    toast.add({
      title: 'Error',
      description: message,
