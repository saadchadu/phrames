...
This document defines the requirements for implementing a paid-only campaign activation system in the Phrames application. The system ensures that all campaigns require payment to become active, automatically deactivates campaigns when their subscription expires, and integrates with Cashfree payment gateway for processing transactions. The feature includes five fixed-duration pricing plans, automatic expiry management via scheduled functions, and a complete reactivation flow for expired campaigns.
...
### Requirement 2: Campaign Publication and Payment Flow

**User Story:** As a creator, I want to be required to pay before my campaign goes live, so that the platform maintains a paid-only model.

#### Acceptance Criteria

1. WHEN a creator clicks the Publish button on a new campaign, THE Phrames_System SHALL display the Payment_Modal with all available pricing plans
2. THE Phrames_System SHALL create campaigns with isActive set to false by default
...
### Requirement 3: Campaign Data Model

**User Story:** As a system administrator, I want campaign documents to store complete payment and expiry information, so that the system can manage activation status accurately.

#### Acceptance Criteria

1. THE Phrames_System SHALL store the following fields in each campaign document: planType, amountPaid, paymentId, expiresAt, isActive, and status
2. THE Phrames_System SHALL validate that planType contains one of: "week", "month", "3month", "6month", or "year"
3. THE Phrames_System SHALL store amountPaid as a number representing the payment amount in rupees
4. THE Phrames_System SHALL store expiresAt as a Firebase Timestamp
5. THE Phrames_System SHALL set status to "Active" when isActive is true and "Inactive" when isActive is false

### Requirement 4: Cashfree Payment Integration
...
### Requirement 5: Automatic Campaign Expiry

**User Story:** As a system administrator, I want campaigns to automatically deactivate when their subscription expires, so that only paid campaigns remain active without manual intervention.

#### Acceptance Criteria

1. THE Phrames_System SHALL implement a Cloud_Function that executes daily
2. WHEN the Cloud_Function executes, THE Phrames_System SHALL query all campaigns where isActive equals true and expiresAt is less than the current timestamp
3. THE Phrames_System SHALL update each expired campaign by setting isActive to false and status to "Inactive"
4. THE Phrames_System SHALL log expiry actions to a Firestore collection for audit purposes
5. THE Phrames_System SHALL complete the expiry check within 5 minutes of the scheduled execution time

### Requirement 6: Campaign Reactivation

**User Story:** As a creator, I want to renew my expired campaign by purchasing a new plan, so that my campaign can become active again.

#### Acceptance Criteria

1. WHEN a creator views an expired campaign in the dashboard, THE Phrames_System SHALL display a message indicating the campaign has expired
2. THE Phrames_System SHALL display a "Renew Plan" button for expired campaigns
3. WHEN a creator clicks "Renew Plan", THE Phrames_System SHALL display the Payment_Modal with all pricing plans
...
5. THE Phrames_System SHALL redirect the creator to the campaign dashboard with a success message after reactivation

### Requirement 7: Public Campaign Visibility
...
1. WHEN a visitor accesses /campaign/[slug], THE Phrames_System SHALL verify that isActive equals true and expiresAt is greater than the current timestamp
2. IF the campaign is inactive or expired, THEN THE Phrames_System SHALL return a 404 status or display the message "This campaign is inactive. Please contact the creator."
3. THE Phrames_System SHALL allow access to the campaign page only when both activation and expiry conditions are met
4. THE Phrames_System SHALL perform the visibility check on every campaign page request

### Requirement 8: Dashboard Campaign Management
...
2. WHEN a campaign is active, THE Phrames_System SHALL display the expiry date in countdown format
3. WHEN a campaign is active, THE Phrames_System SHALL display "Edit" and "View Campaign" buttons
4. WHEN a campaign is inactive, THE Phrames_System SHALL display a "Reactivate Campaign" button
5. WHEN a creator clicks "Reactivate Campaign", THE Phrames_System SHALL display the Payment_Modal

### Requirement 9: Payment Security and Data Integrity

**User Story:** As a system administrator, I want payment-related fields to be protected from unauthorized modification, so that the system maintains data integrity and prevents fraud.

#### Acceptance Criteria

1. THE Phrames_System SHALL restrict direct modification of isActive, expiresAt, planType, and amountPaid fields by users
2. THE Phrames_System SHALL allow only the Webhook handler and Cloud_Function to update payment-related fields
3. WHEN initiating a payment, THE Phrames_System SHALL validate that the requesting user owns the campaign
4. THE Phrames_System SHALL verify Cashfree webhook signatures before processing payment notifications
5. THE Phrames_System SHALL implement Firestore security rules that prevent client-side updates to payment fields

### Requirement 10: Payment Modal User Interface
...
### Requirement 11: Payment Error Handling
